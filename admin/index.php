...
<section class="relative h-[70vh] sm:h-[80vh] overflow-hidden">
  ...
  <div class="relative z-10 h-full flex flex-col items-center justify-center text-center px-4">
    ...
    <img src="/hatrox-project/assets/images/icons/icon.png" alt="HATROX Logo" class="h-12 sm:h-16 w-auto object-contain -ml-1 m-0 p-0">
    </h1>
    ...
    <a href="/hatrox-project/admin/dashboard.php" class="mt-8 inline-block bg-gold text-black px-6 py-3 rounded hover:bg-rose transition">Go to Dashboard</a>
  </div>
</section>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
    <?php foreach ($featured as $p): $price=(float)$p['price']; $disc=(int)($p['discount_percent']??0); $new=$disc>0?$price*(1-$disc/100):$price; ?>
      <a href="/hatrox-project/admin/products.php?id=<?= (int)$p['id'] ?>" class="group block bg-neutral-900 border border-neutral-800 rounded overflow-hidden hover:shadow-xl hover:shadow-gold/10 transition">
        ...
        <div class="overflow-hidden group [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
          <div class="flex gap-4 animate-marquee group-hover:[animation-play-state:paused] will-change-transform">
            <?php for ($loop=0;$loop<2;$loop++): foreach ($list as $p): $price=(float)$p['price']; $disc=(int)($p['discount_percent']??0); $new=$disc>0?$price*(1-$disc/100):$price; ?>
              <a href="/hatrox-project/admin/products.php?id=<?= (int)$p['id'] ?>" class="min-w-[300px] max-w-[300px] bg-neutral-900 border border-neutral-800 rounded overflow-hidden hover:shadow-lg hover:shadow-gold/10 transition relative">
